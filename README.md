...
Usage keeps detailed half-hour readings for up to 120 days and compact local daily totals for up to five years. The longer views compare complete periods and deliberately avoid attributing seasonal changes to weather or another cause. This account-specific archive stays in the application's local cache and is replaced when the configured account changes. Available history can be shorter after moving home or replacing a meter because the app does not combine independent supplies or meter serials.
